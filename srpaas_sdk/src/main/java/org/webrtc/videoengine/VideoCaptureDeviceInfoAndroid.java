...
public class VideoCaptureDeviceInfoAndroid {
	...
	public static VideoCaptureDeviceInfoAndroid CreateVideoCaptureDeviceInfoAndroid(
			int in_id, Context in_context) {
...
//					newDevice.deviceUniqueName = "Camera " + i
//							+ ", Facing back, Orientation " + info.orientation;
//					// Log.d(TAG, "Raycom...Camera " + i
//					// +", Facing back, Orientation "+ info.orientation);
//
//				} else {
//					newDevice.deviceUniqueName = "Camera " + i
//							+ ", Facing front, Orientation " + info.orientation;
...
//		VerifyCapabilities();
	...
	public VideoCaptureAndroid AllocateCamera(int id, long context,
		...
		for (AndroidVideoCaptureDevice device : deviceList) {
			// Log.d(TAG, "Raycom...AllocateCamera....22.. "
			// +"...."+device.frontCameraType);
			if (device.deviceUniqueName.equals(deviceUniqueId)) {
		...
		if (deviceToUse == null) {
			Log.v(TAG,
					"Raycom...AllocateCamera - creating VideoCaptureAndroid error");
			return null;
		}
		Log.v(TAG, "Raycom...AllocateCamera - creating VideoCaptureAndroid");
	...
	private Camera.Parameters SearchOldFrontFacingCameras(
			AndroidVideoCaptureDevice newDevice) throws SecurityException,
		...
		File file = new File(
				"/system/framework/com.htc.hardware.twinCamDevice.jar");
		boolean exists = file.exists();
		if (!exists) {
			file = new File(
					"/system/framework/com.sprint.hardware.twinCamDevice.jar");
			exists = file.exists();
		}
		if (exists) {
	...
	@SuppressLint("NewApi")
	private Camera AllocateEVOFrontFacingCamera() throws SecurityException,
